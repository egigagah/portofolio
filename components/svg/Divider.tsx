...
const DividerSvg: FC<DividerProps> = ({isTop = false, height = 'h-[125px] md:h-[235px]', ...rest}) => {
  return (
    <div className={`absolute ${isTop ? 'top-0' : 'bottom-0'} left-0 w-full overflow-hidden leading-none ${rest.className}`}>
      <svg id={`divider-svg-${rest.id??0}`} className={`relative block w-[calc(300%+1.3px)] ${height}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
